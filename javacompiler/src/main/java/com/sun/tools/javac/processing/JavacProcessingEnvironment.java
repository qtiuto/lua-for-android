...
public class JavacProcessingEnvironment implements ProcessingEnvironment, Closeable {
    ...
    private void initProcessorIterator(Iterable<? extends Processor> processors) {
        ...
        if (platformProvider != null) {
            platformProcessors = StreamSupport.stream(platformProvider.getAnnotationProcessors())
    ...
    class Round {
        ...
        private Round(Round prev,
            ...
            for (Entry<ModuleSymbol, Map<String, JavaFileObject>> moduleAndClassFiles : newClassFiles.entrySet()) {
                Maps.computeIfAbsent(genClassFiles,moduleAndClassFiles.getKey(), m -> new LinkedHashMap<>()).putAll(moduleAndClassFiles.getValue());
            }
            enterTrees(roots);
