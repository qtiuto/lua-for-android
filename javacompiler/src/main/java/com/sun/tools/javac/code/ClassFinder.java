 ...
 ***********************************************************************/
    ...
    protected void includeClassFile(PackageSymbol p, JavaFileObject file) {
        ...
        int seen;
        if (kind == JavaFileObject.Kind.CLASS||kind==Kind.DEX_CALSS || kind == JavaFileObject.Kind.OTHER)
            seen = CLASS_SEEN;
        else
            seen = SOURCE_SEEN;
    ...
    protected EnumSet<JavaFileObject.Kind> getPackageFileKinds() {
        return EnumSet.of(JavaFileObject.Kind.CLASS, Kind.DEX_CALSS, JavaFileObject.Kind.SOURCE);
    }
    ...
    private void scanModulePaths(PackageSymbol p, ModuleSymbol msym) throws IOException {
        ...
        sourceKinds.remove(Kind.DEX_CALSS);
        ...
        if (verbose && verbosePath) {
            if (fileManager instanceof StandardJavaFileManager) {
                ...
                if (haveSourcePath && wantSourceFiles) {
                    ...
                    for (File sourcePath : fm.getLocation(SOURCE_PATH)) {
                        path = path.prepend(sourcePath);
                    }
                    log.printVerbose("sourcepath", path.reverse().toString());
                } else if (wantSourceFiles) {
                    List<File> path = List.nil();
                    for (File classPath : fm.getLocation(CLASS_PATH)) {
                ...
                if (wantClassFiles) {
                    List<File> path = List.nil();
                    for (File platformPath : fm.getLocation(PLATFORM_CLASS_PATH)) {
