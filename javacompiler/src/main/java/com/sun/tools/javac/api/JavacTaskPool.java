...
public class JavacTaskPool {
    ...
    public <Z> Z getTask(Writer out,
                         ...
                         Worker<Z> worker) {
        List<String> opts =
                StreamSupport.stream(Iterables.spliterator(options), false)
        ...
        synchronized (this) {
            List<ReusableContext> cached =
                    Maps.getOrDefault(options2Contexts,opts, Collections.emptyList());
            ...
            synchronized (this) {
                while (cacheSize() + 1 > maxPoolSize) {
                    ReusableContext toRemove =
                                            StreamSupport.stream(options2Contexts.values())
                ...
                Maps.computeIfAbsent(options2Contexts,ctx.arguments, x -> new ArrayList<>()).add(ctx);
                ctx.timeStamp = id++;
            }
        }
    ...
    //where:
        private long cacheSize() {
            return StreamSupport.stream(options2Contexts.values()).flatMap(StreamSupport::stream).count();
        }
