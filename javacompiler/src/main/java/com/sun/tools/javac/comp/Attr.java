 ...
 *************************************************************************/
    ...
    boolean checkExConstraints(
         ...
         */
        List<Type> nonProperList =StreamSupport.stream(thrownAtFuncType)
                .filter(inferenceContext::free).collect(List.collector());
         ...
         *  declaration
         */
        List<Type> checkedList = StreamSupport.stream(thrownByFuncExpr)
                .filter(chk::isChecked).collect(List.collector());
        ...
        StreamSupport.stream(uncaughtByProperTypes).forEach(checkedEx -> {
            StreamSupport.stream(nonProperAsUndet).forEach(nonProper -> {
         ...
         */
        StreamSupport.stream(nonProperAsUndet)
