...
public class Types {
    ...
    class TypeProjection extends StructuralTypeMapping<ProjectionKind> {
        ...
        public Type visitClassType(ClassType t, ProjectionKind pkind) {
            if (t.isCompound()) {
                ...
                if (StreamSupport.stream(typarams1).anyMatch(ta -> ta.hasTag(BOT))) {
                    //not defined
                    return syms.botType;
                }
    ...
    class CaptureScanner extends SimpleVisitor<Void, Set<Type>> {
        ...
        public Void visitClassType(ClassType t, Set<Type> seen) {
            if (t.isCompound()) {
                for (Type s:directSupertypes(t)) visit(s, seen);
            } else {
                for (Type ta:t.allparams()) visit(ta, seen);
            }
            return null;
        }
        ...
        public FunctionDescriptor findDescriptorInternal(TypeSymbol origin,
            ...
            for (Symbol sym : membersCache.getSymbols(new DescriptorFilter(origin))) {
                ...
                if (abstracts.isEmpty()) {
                    ...
                    if (!StreamSupport.stream(abstracts).filter(msym -> msym.owner.isSubClass(sym.enclClass(), Types.this))
    ...
    public Type removeWildcards(Type site) {
        if (StreamSupport.stream(site.getTypeArguments()).anyMatch(t -> t.hasTag(WILDCARD))) {
...
//                    debugContainsType(t, s);
     ...
     * {@link TypeSymbol#precedes(TypeSymbol,Types)}.
     *
