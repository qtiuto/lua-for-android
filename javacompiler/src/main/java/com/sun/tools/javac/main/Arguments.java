    ...
    public boolean validate() {
        ...
        if (fm instanceof StandardJavaFileManager) {
            ...
            if (sfm.hasLocation(StandardLocation.CLASS_OUTPUT)) {
                File outDir = sfm.getLocation(StandardLocation.CLASS_OUTPUT).iterator().next();
                if (sfm.hasLocation(StandardLocation.MODULE_SOURCE_PATH)) {
                    // multi-module mode
                    if (new File(outDir,"module-info.class").exists()) {
                    ...
                    if (lintPaths) {
                        String outDirParent = outDir.getParent();
                        if (outDirParent != null && new File(outDirParent,"module-info.class").exists()) {
