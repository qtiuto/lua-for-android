...
public enum Option {
        ...
        public void process(OptionHelper helper, String option, String arg) throws InvalidValueException {
            if (arg.isEmpty()) {
                ...
                if (prev == null) {
                    ...
                    boolean isRepeated = StreamSupport.stream(Arrays.asList(prev.split("\0")))
    ...
    RELEASE("--release", "opt.arg.release", "opt.release", STANDARD, BASIC) {
        ...
        protected void help(Log log) {
            ...
            Set<String> platforms = StreamSupport.stream(Iterables.spliterator(providers), false)
                                                 .flatMap(provider -> StreamSupport.stream(Iterables.spliterator(provider.getSupportedPlatformNames()),
                                                                                           false))
    ...
    INHERIT_RUNTIME_ENVIRONMENT("--inherit-runtime-environment", "opt.inherit_runtime_environment",
                        ...
                        if (o.matches(arg)) {
                            switch (o) {
                                case ADD_MODULES:
                                    ...
                                    String mods = StreamSupport.stream(Arrays.asList(arg.substring(eq + 1).split(",")))
                                            .filter(s -> !s.isEmpty() && !s.equals("ALL-DEFAULT"))
                                            .collect(Collectors.joining(","));
                                    if (!mods.isEmpty()) {
                                        String updatedArg = arg.substring(0, eq + 1) + mods;
                                        o.handleOption(helper, updatedArg, emptyIterator());
                                    }
                                    break;
                                default:

                                    o.handleOption(helper, arg, emptyIterator());
                                    break;
                            }
                            break;
                        }
        ...
        private  <T> Iterator<T> emptyIterator(){
...
//        OPTIONAL,
    ...
    private static void showHelp(Log log, OptionKind kind) {
        ...
        StreamSupport.stream(getJavaCompilerOptions())
    ...
    private static Set<Option> getOptions(OptionGroup group) {
        return StreamSupport.stream(Arrays.asList(Option.values()))
